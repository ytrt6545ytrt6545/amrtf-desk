...
- [x] **Firebase 雲端中繼、100% 密碼學純掃碼直通與 100 間研討教室多租戶隔離（含原生 RTDB 穿透、段落 markers 同步與去固化下課即焚）**：
  ...
  - **全鏈路狀態對齊與手抄稿段落 markers 同步**：放映艙 20+ 個黃金手抄稿段落秒數完整推播至雲端與手機端下拉選單，點按播放/暫停/跳段微秒級連鎖響應；
  - **100 間教室多租戶隔離、不固化房間與下課即焚 (Zero-Garbage)**：恪守長官指示，絕不在硬碟固化房間，每次啟動全新動態生成 32 碼 Token 與 8 碼 Room ID；電腦端關閉時觸發 `/api/shutdown` 優雅停機，優先自毀抹除雲端房間資料，零歷史垃圾積累；
  - **單一 SPA 部署於 Firebase Hosting (`src/mobile-client/`)**：全球 CDN 託管，支援螢幕常亮（Screen Wake Lock API）、微震反饋與本地高頻碼表計時；
  - **主控台雙軌 QR Modal (`src/desk/`)**：預設雲端純掃碼，並提供 LAN 區域網路備援切換。
...
- [x] **大慈恩宣紙明亮／玄木暗黑雙風格系統與持久化閉環 (亮點 115)**：
  - **大慈恩書卷美學體系**：依長官指示全面適配大慈恩官方氣質，提供「宣紙明亮 (Light Parchment · 預設)」與「玄木禪境 (Dark Zen)」雙風格切換；
  - **廣播級 Active-Deep 極致高反差**：明亮模式下按鈕啟用狀態反轉為深檀墨底（`#2B2521`）＋ 佛金發光字（`#E5A93C`），白底上一眼辨識，徹底杜絕誤觸；
  - **全域 Tokens 一體化適配與 0.25s 柔和轉場**：全域覆蓋 LED 碼表盤、提詞機、按鈕陣列、起訖選單、自訂按鈕倉庫與設定抽屜，切換瞬間絲滑舒適；
  - **右上角微按鈕與 LocalStorage 永續記憶**：頂部導航列配置 🌞／🌙 微按鈕，切換即時寫入 `localStorage.amrtf_theme`，開機防閃爍內聯腳本零延遲還原；
  - **Live Reality E2E 四重硬鎖物證驗證**：新增 `[E2E-9]` 測試，CDP 實機點擊驗證計算樣式背景色實質突變（$\Delta \neq 0$）並捕獲真機明暗雙快照存證。
- [x] **放映端 🌓 主題信令徹底解耦 ＋ 系統版本號與 GitHub 自動更新熱拉取閉環 (亮點 116)**：
  - **雙主題按鈕徹底解耦**：頂部主控台按鈕獨立命名為 `#btnDeskThemeToggle`，下方鍵盤 `#btnThemeToggle`（🌓）100% 歸還 `sendCmd('set_theme')`，修復放映艙手抄稿深淺切換；
  - **版本號與設定艙集成**：設定艙常駐版本卡片（`v1.0.0`），有新版時頂部齒輪亮起小金色紅點提醒；
